...
const Header: React.FC<{ rightHeader?: boolean }> = ({ rightHeader = false }) => {
  ...
  return (
    <header className="layout-p sticky top-0 z-40 w-full bg-white shadow-[0_1px_5px_rgba(0,0,0,.3)]">
      <div className="relative z-10 mx-auto max-w-screen-2xl bg-white">
        <div className="flex items-center bg-white py-4">
          ...
          {!login && (
            <Link
              to={'/login'}
              className={`min-[325px]:ml-4 block px-2 py-4 hover:text-clr-ButtonRed md:hidden ${pathname === '/login' && 'text-clr-Secondary2 underline'}`}
              onClick={() => setOpenNav(false)}
            >
              Login
            </Link>
